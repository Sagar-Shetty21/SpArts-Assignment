    ...
    return (
        <div className="border rounded bg-white w-full mt-1">
            <div className="flex flex-wrap md:flex-nowrap items-center border">
                <div className="text-lg font-medium text-left p-2 flex-grow ml-2 order-1">UPCOMING SCHEDULE</div>
                <div className="flex items-center justify-center lg:justify-start p-1 pr-2 flex-grow text-white order-3 md:order-2">
                    <CiFilter className="text-2xl text-gray-600"/>
                    <div className={`p-1 w-20 md:w-28 m-2 rounded-lg text-center cursor-pointer ${currentFilter === 'All' ? 'bg-slate-600' : 'bg-slate-400'}`} onClick={() => handleClick("All")}>All</div>
                    <div className={`p-1 w-20 md:w-28 m-2 rounded-lg text-center cursor-pointer ${currentFilter === 'Classes' ? 'bg-slate-600' : 'bg-slate-400'}`} onClick={() => handleClick("Classes")}>Classes</div>
                    <div className={`p-1 w-20 md:w-28 m-2 rounded-lg text-center cursor-pointer ${currentFilter === 'Meetings' ? 'bg-slate-600' : 'bg-slate-400'}`} onClick={() => handleClick("Meetings")}>Meetings</div>
                    <div className={`p-1 w-20 md:w-28 m-2 rounded-lg text-center cursor-pointer ${currentFilter === 'Events' ? 'bg-slate-600' : 'bg-slate-400'}`} onClick={() => handleClick("Events")}>Events</div>
                </div>
            ...
            <div className="border">
                {currentTableData.map((data, index) => (
                    <div key={index} className="grid grid-cols-10 gap-1 w-full items-center p-2 justify-between">
                        ...
                        <div className="col-span-4 lg:col-span-3 text-gray-500 font-bold text-left">{data.subject}</div>
